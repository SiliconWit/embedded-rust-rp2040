...
stateDiagram-v2
    [*] --> OwnedByCPU : Buffer allocated
    OwnedByCPU --> OwnedByDMA : uart.read(&mut buf).await
    note right of OwnedByDMA : Compiler rejects any\nread/write to buf here.\nCompile error, not runtime crash.
    OwnedByDMA --> OwnedByCPU : .await completes\n(transfer done)
    OwnedByCPU --> Processing : Parse NMEA data
    Processing --> OwnedByCPU : Ready for next transfer
```
