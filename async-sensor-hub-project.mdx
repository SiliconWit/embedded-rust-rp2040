...
Individual peripherals are straightforward in isolation. The real challenge appears when you need a sensor reading every 100 ms, an OLED refresh every 500 ms, an SD card write every second, and an MQTT publish every two seconds, all without any task starving the others or corrupting shared data. In C with FreeRTOS, you would manage six threads, six stacks, and a web of mutexes. Embassy collapses this into six async tasks sharing a single stack, with zero-copy channels replacing mutexes and the compiler enforcing that no two tasks can alias the same buffer. In this capstone you will build exactly that system on the Pico W: BME280 readings flowing through channels to a display, an SD logger, an MQTT publisher, button handlers, and status LEDs, all running concurrently with no shared mutable globals. #AsyncEmbedded #SensorHub #Capstone
...
```text
  Pico W Sensor Hub Wiring
  ──────────────────────────────────────────
  GP4 (SDA0) ──── BME280 SDA     (I2C0 sensor)
  GP5 (SCL0) ──── BME280 SCL

  GP6 (SDA1) ──── SSD1306 SDA    (I2C1 display)
  GP7 (SCL1) ──── SSD1306 SCL

  GP10 (SCK)  ──── SD Card CLK   (SPI1 storage)
  GP11 (MOSI) ──── SD Card MOSI
  GP12 (MISO) ──── SD Card MISO
  GP13 (CS)   ──── SD Card CS

  GP16 ──330R──── Green LED      (Wi-Fi status)
  GP17 ──330R──── Yellow LED     (SD activity)

  GP18 ──── Button A (to GND)    (display mode)
  GP19 ──── Button B (to GND)    (manual publish)

  3V3 ──── All component VCC
  GND ──── All component GND
```
