...
A sensor node tethered to USB can only report data to the machine it is plugged into. Adding Wi-Fi lets it publish readings to any MQTT broker on the network, but networking on a microcontroller without an OS means you need a TCP/IP stack, a Wi-Fi driver, and an async runtime that can juggle socket I/O alongside sensor reads without blocking either one. Embassy provides all three in Rust: the `cyw43` crate drives the Pico W's wireless chip, `embassy-net` handles TCP/IP, and the async executor keeps everything running concurrently on a single core. In this lesson you will connect to Wi-Fi, open TCP sockets, send a raw HTTP request, and build an MQTT sensor node that publishes BME280 readings every 10 seconds with automatic reconnection. #PicoW #EmbassyNet #MQTT
...
stateDiagram-v2
    [*] --> ConfigPins : PIO + SPI pins
    ConfigPins --> LoadFirmware : Create cyw43 driver
    LoadFirmware --> BackgroundTask : Spawn cyw43 task
    BackgroundTask --> Joining : control.join_wpa2()
    Joining --> Connected : WPA2 handshake OK
    Joining --> Joining : Retry after backoff
    Connected --> DHCPWait : Start DHCP
    DHCPWait --> Ready : IP assigned
    Ready --> [*]
```
