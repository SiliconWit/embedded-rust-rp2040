...
Most microcontrollers need a separate UART-to-USB bridge chip (an FTDI or CP2102) just to send serial data to a computer, and implementing USB device classes in C means hand-packing descriptor byte arrays where a single off-by-one error produces a device that silently fails to enumerate. The RP2040 has a built-in USB 1.1 controller with an integrated PHY, so it can act as a USB device without any external chip. Plug it into a computer and it can appear as a serial port, a keyboard, a mouse, or all of these at once. In this lesson you will use embassy-usb to build a USB CDC virtual serial port that streams sensor data, a USB HID keyboard that sends keystrokes when you press a button, and finally a composite device that combines both. The embassy-usb builder pattern makes descriptor construction type-safe, catching at compile time the layout errors that would take hours to debug with a protocol analyzer in C. #USB #EmbassyRust #HID
...
```text
  USB Descriptor Tree (Composite Device)
  ──────────────────────────────────────────
  Device Descriptor
  ├── VID: 0xCAFE, PID: 0x0001
  └── Configuration 1 (max 100 mA)
      ├── Interface 0: CDC ACM (Communication)
      ...
      └── Interface 2: HID Keyboard
          └── Endpoint 3 IN (Interrupt, 8 bytes)
```
